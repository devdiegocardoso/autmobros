...
public:
    OrientationException(std::string name, eeros::sequencer::Sequence *caller, 
        ...
        cs(cs), checkOrientation(checkOrientation), eeros::sequencer::Sequence(name, caller,true)
        {
            log.info() << "Sequence created: " << name;
        }

    int action(){
        log.warn() << "Orientation around x is outside limits";
        log.warn() << "Stopping all motors!";
